...
namespace TaoTie.Inspector.Editor
{
    public abstract partial class GraphWindow  : EditorWindow  
    {
        ...
        protected Dictionary<string, List<VirtualPoint>> m_Points;
        protected Dictionary<string, Port> m_Ports;
        protected Dictionary<string, EdgeView> m_EdgeViews;
        ...
        protected bool m_PointsDirty = true;
        ...
        private const float k_MinInspectorWidth = 400f;
        private bool m_InspectorResizing;
        private Rect m_InspectorDragRect;
                ...
                foreach (var nodeView in nodeViews.Values)
                {
                    if (nodeView.node.inputPorts != null)
                    {
                        foreach (var port in nodeView.node.inputPorts)
                        {
                            ...
                            if (m_Points.ContainsKey(port.id)) continue;
        ...
        protected virtual void OnEnable()
        {
            ...
            AddButton(new GUIContent("Open"), LoadGraph);
            AddButton(new GUIContent("Save"), SaveGraph);
            AddButton(new GUIContent("Toggle Node Info"), ChangeShowNodeViewDetails);
            AddButton(new GUIContent("Inspector"), ChangeDrawInspector, false);
        ...
        {
            if (m_UndoStack.Count == 0) { ShowNotification(new GUIContent("Nothing to undo"), 1f); return; }
        ...
        {
            if (m_RedoStack.Count == 0) { ShowNotification(new GUIContent("Nothing to redo"), 1f); return; }
            var current = SerializeGraph();
            if (!string.IsNullOrEmpty(current))
        ...
        private void DrawPlayModeLocked()
        {
            ...
            DrawCenteredMessage(area, "Play Mode — editing locked");
        }
        ...
        private void DrawEmptyState()
        {
            ...
            var area = new Rect(0, 20, position.width, position.height - 20);
            GraphBackground.DrawGrid(area, 1f, Vector2.zero);
            DrawCenteredMessage(area, "No graph loaded\nClick \"New\" or \"Open\" in the toolbar");
        }
        ...
        private void DrawViewGraph()
        {
            ...
            if (m_DrawInspector)
            {
                m_InspectorDragRect = new Rect(graphViewArea.width - 4, 0, 8, position.height);
                EditorGUIUtility.AddCursorRect(m_InspectorDragRect, MouseCursor.ResizeHorizontal);
                var e = Event.current;
                if (e != null && m_InspectorDragRect.Contains(e.mousePosition))
                {
                    if (e.type == EventType.MouseDown && e.button == 0)
                    {
                        m_InspectorResizing = true;
                        e.Use();
                    }
                }
                if (m_InspectorResizing)
                {
                    if (e.type == EventType.MouseDrag && e.button == 0)
                    {
                        m_NodeInspectorWidth = Mathf.Max(k_MinInspectorWidth, position.width - e.mousePosition.x);
                        e.Use();
                        Repaint();
                    }
                    if (e.type == EventType.MouseUp && e.button == 0)
                    {
                        m_InspectorResizing = false;
                        e.Use();
                    }
                }
            }
        ...
        /// <summary>Mark the geometry (VirtualPoints / EdgeView curves) as stale and schedule a draw.</summary>
        ...
        private void SyncNodeDrawOrder()
        {
            ...
            for (int i = m_NodeDrawOrder.Count - 1; i >= 0; i--)
            {
                var v = m_NodeDrawOrder[i];
        ...
        /// <summary>Bring the given nodes to the front of the window draw order (last = topmost).</summary>
        private void BringNodesToFront(IEnumerable<NodeBase> nodes)
        {
            SyncNodeDrawOrder();
    ...
    public abstract partial class GraphWindow<T> : GraphWindow where T : GraphBase
    {
        ...
        protected override string SerializeGraph()
        {
            ...
            // Create a temporary instance, copy data via JsonUtility, and serialize
            // This avoids issues with ScriptableObject references in sub-assets
            try
            {
                return JsonUtility.ToJson(m_Graph);
            }
            catch (Exception e)
            {
                Debug.LogError($"SerializeGraph failed: {e.Message}");
                return null;
            }
        }
        ...
        protected override GraphBase DeserializeGraph(string json)
        {
            ...
            // JsonUtility.FromJson<T> creates a new instance of T (must have parameterless constructor)
            // But ScriptableObject types can't be instantiated via FromJson directly.
            // Use CreateInstance + FromJsonOverwrite instead.
            try
            {
            ...
            {
                Debug.LogError($"DeserializeGraph failed: {e.Message}");
                return null;
            }
        }
    }
