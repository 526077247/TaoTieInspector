...
namespace TaoTie.Inspector.Editor
{
    public abstract partial class GraphWindow
    {
        ...
        private VirtualPoint GetVirtualPointAtWorldPosition(Vector2 worldPosition)
        {
            ...
            float size = 12;
            var mouseRect = new Rect(worldPosition.x, worldPosition.y, size, size);
            mouseRect.x -= size / 2;
            mouseRect.y -= size / 2;

            foreach (var key in points.Keys)
            {
                if (!points.TryGetValue(key, out var pointList)) continue;
                foreach (var point in pointList)
                {
                    if (!nodeViews.TryGetValue(point.node.id, out var nv) || nv == null || !nv.isVisible)
                        continue;
                    // VP rect already contains panOffset (baked in at creation time)
                    // Convert to screen space: rect * zoom + graphAreaOffset
                    var screenPos = point.rect.position * currentZoom;
                    var screenSize = point.rect.size * currentZoom;
                    var pointScreenRect = new Rect(screenPos.x, screenPos.y, screenSize.x, screenSize.y);
                    pointScreenRect.x -= pointScreenRect.width / 2;
                    pointScreenRect.y -= pointScreenRect.height / 2;
                    if (pointScreenRect.Overlaps(mouseRect)) return point;
                }
            }
        ...
        protected virtual void ShowPortContextMenu(Port port, bool isLine = false)
        {
            ...
            {
                var res = EditorUtility.DisplayDialog("Confirm", "Disconnect this edge?", "Yes", "No");
                ...
                if (m_Mode == GraphMode.Connect)
                {
                    ...
                    else
                    {
                        m_CreateEdgeLineColor = m_ActivePort.IsInput()
                            ? NodeColors.EdgeInput
                            : NodeColors.EdgeOutput;
                    }
        ...
        protected virtual void RemovePort(Port port,bool force = false)
        {
            ...
            PushUndoSnapshot();
            DisconnectPort(port);
            if (m_Points != null && m_Points.ContainsKey(port.id))
                m_Points.Remove(port.id);
            if (m_EdgeViews != null)
            {
                var keysToRemove = new List<string>();
                foreach (var kvp in m_EdgeViews)
                {
                    if (kvp.Value.outputPort == port || kvp.Value.inputPort == port)
                        keysToRemove.Add(kvp.Key);
                }
                foreach (var key in keysToRemove)
                    m_EdgeViews.Remove(key);
            }
            m_Ports = null;
            m_PointsDirty = true;
            m_Dirty = true;
