        ...
        public void RemoveEdge(string edgeId)
        {
            if (!string.IsNullOrEmpty(edgeId))
            {
                for (int i = 0; i < edges.Count; i++)
                {
                    ...
                    {
                        var edge = edges[i];
                        // Clean up port references before destroying the edge
                        var inputNode = FindNode(edge.inputNodeId);
                        if (inputNode != null && inputNode.inputPorts != null)
                        {
                            foreach (var port in inputNode.inputPorts)
                            {
                                if (port != null && port.id == edge.inputPortId)
                                {
                                    port.RemoveEdge(edgeId);
                                    break;
                                }
                            }
                        }
                        var outputNode = FindNode(edge.outputNodeId);
                        if (outputNode != null && outputNode.outputPorts != null)
                        {
                            foreach (var port in outputNode.outputPorts)
                            {
                                if (port != null && port.id == edge.outputPortId)
                                {
                                    port.RemoveEdge(edgeId);
                                    break;
                                }
